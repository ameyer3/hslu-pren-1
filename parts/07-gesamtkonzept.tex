...
Es ist geplant einen Roboter in in Form eines freien Ploygons zu bauen, der sich mit drei Rädern fortbewegt und eine Motorlenkung besitzt. Hindernisse kann der Roboter mit einem Greifer mit flexiblen Backen anheben. Dies ist ersichtlich in der vorherigen Abbildung \ref{fig:robot_concept-scetch_labeld}.

Die Steuerung wird auf einem \acrshort{tinyk22} laufen. Die Distanz zu den Objekten wird mit Ultraschall erkannt. Die bürstenbehafteten Motoren werden mit einem gekauften Treiber angesteuert. Die Stromversorgung läuft über einen Akku. Der Akkustand und der Status des Roboters werden mit \acrfull{led} angezeigt. Damit der Roboter die Linien erkennt, wird ein Liniensensor mit Fototransistoren verwendet. Das Ziel wird über einen Schalter vom Benutzer ausgewählt.
...
Dadurch dass \acrshort{orb} die Merkmal der Buchstaben detektiert, ist es nicht nötig, die Rotation der Buchstaben zu beachten. Unabhängig von welcher Richtung der Roboter den Zielknoten fotografiert, kann der Buchstabe erkannt werden.
...
Um ein vorhandenes Hindernis zu bewegen wird ein Mechanismus zum Anheben (nachfolgend Greifer genannt) und das Fahrzeug selbst zum verschieben des Hindernisses verwendet.
...
Das Fahrzeug wird mit zwei seperat angetriebenen Räder angetrieben. Die Motoren besitzen Encoder, somit kann man genau feststellen welche Distanz zurückgelegt wurde. Der Fahrbefehl entsteht aufgrund der Berechnungen der Bilderkennung. Dem Mikrocontroller \acrshort{tinyk22} werden Distanz und Drehwinkel mitgeteilt, wodurch dann die Motoren angesteuert werden können.
