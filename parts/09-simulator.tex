...
Falls noch nie eine Berechnung des kürzesten Weges durchgeführt wurde oder sich der intern gespeicherte Graph verändert hat, wird die Berechnung des kürzesten Pfades mittels \gls{dijkstra} durchgeführt. Der geplante Pfad sowie der nächste Knoten werden dabei gespeichert.
...
Dabei ist wichtig, dass wir unter einer Laufzeit von ca. 2 Minuten stehen, egal welcher Graph mit welchen Hindernissen traversiert wird.

Nachfolgend in Abbildung \ref{fig:simulation-run-chart} sind diese Durchläufe und deren Laufzeiten in einer Grafik Dargestellt. Die Simulationen sind nach Anzahl Hindernissen gruppiert. Also beginnend mit 0 Pylonen, 0 Barrieren und 0 fehlende Kanten. Und enden mit der maximalen Anzahl Pylonen, Barrieren und fehlenden Kanten. Also 5 Pylonen, 15 Barrieren und 13 fehlende Kanten.
Die Anzahl Hindernissen ist dabei im unteren Liniendiagramm ersichtlich. Das darüberliegende Liniendiagramm zeigt dann die dazu erreichten maximalen, minimalen und durchschnittlichen Laufzeit.
...
Wie zu sehen ist, braucht der Roboter im ungünstigsten Fall ungefähr 200 Sekunden für das Traversieren. Welches immer noch unter der geforderten Muss-Anforderung von 240 Sekunden ist. 
