...
\textbf{} & \textbf{Selber implementieren} & \textbf{Externe Library verwenden}\\
  \hline
  \textbf{Beschreibung} & Der Algorithmus zur Wegfindung wird manuell implementiert.  & Es wird eine externe Library verwendet, um den Algorithmus auszufuehren.  \\
  \hline
  \textbf{Vorteile}  & \makecell{-keine externen Abhaengigkeiten\\-keine unnoetigen Zusatzfunktionen} & \makecell{-simpel\\-wahrschieilich effizienter\\-weniger Fehleranfaellig} \\
  \hline
  \textbf{Nachteile} & \makecell{-komplex} & \makecell{-externe Abhaengigkeit} \\
  \hline
...
\textbf{} & \textbf{Human Input} & \textbf{Roboter berechnet}\\
  ...
  \textbf{Vorteile}  & \makecell{-lightweight} & \makecell{-Gewichtung moeglich} \\
  \hline
  \textbf{Nachteile} & \makecell{-kann die Kanten nicht gewichten} & \makecell{-aufwaendig\\-moeglicherweise langsam} \\
  \hline
  \textbf{Links} & \url{https://www.python.org/doc/essays/graphs/} & \url{https://de.wikipedia.org/wiki/Adjazenzmatrix}\\
  \hline
\end{tabularx}
