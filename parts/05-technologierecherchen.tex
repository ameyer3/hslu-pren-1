...
\subsubsection{Fahrwerk/Getriebe/Lenkung}

\begin{huge}
\color{red}
    Eine der Nachfolgenden "Fahrwerkstypen" Tabellen auswählen, die andere löschen!!
\end{huge}

\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{|p{48mm}|l|l|X|}
\hline
    \textbf{Typ} & \textbf{Vorteile} & \textbf{Nachteile} & \textbf{Quelle} \\
\hline
    \textbf{Differential Wheel Drive} \newline 2 einzeln Angetriebene Räder, 1 Schwenkrad 
    & \makecell{-kein Lenkgestänge\\-leicht\\-Punktwendung möglich} & \makecell{-instabil\\-2 Antriebsmotoren} 
    & \url{https://en.wikipedia.org/wiki/Differential_wheeled_robot}\\
\hline
    \textbf{Überlagerungslenkgetriebe} \newline "Panzerlenkung" 1 Motor mit 2 Ausgangswellen, Drehzahl/Richtung unabhängig einstellbar
    & \makecell{-1 Antriebsmotor\\-1Lenkmotor\\-kein Lenkgestänge}
    & \makecell{-Getriebe mit Verlusten\\-komplexer Aufbau}
    & \url{https://de.wikipedia.org/wiki/Überlagerungslenkgetriebe} \\
\hline
    \textbf{Lenkgetriebe} \newline zur Umwandlung Rotation in unterschiedliche Einschlagwinkel
    & \makecell{-simpler Aufbau\\-1 Aktuator}
    & \makecell{-begrenzter Einschlag}
    & \url{https://de.wikipedia.org/wiki/Lenkgetriebe}\\
\hline
\end{tabularx}
\caption{Fahrwerkstypen}
\label{table:chassis/transmission-types}
\end{table}
...
\textbf{} & \textbf{Lenkgetriebe} & \textbf{Omniwheel}\\
  \hline
  \textbf{Beschreibung} & zur Umwandlung Rotation in unterschiedliche Einschlagwinkel & Rad, welches sich zusätzlich zum \\
  \hline
  \textbf{Vorteile} & \makecell{-simpler Aufbau\\-1 Aktuator} & \\
  \hline
  \textbf{Nachteile} & \makecell{-begrenzter Einschlag} & \\
  \hline
  \textbf{Links} & \url{https://de.wikipedia.org/wiki/Lenkgetriebe} & \\
  \hline
\end{tabularx}
\caption{Fahrwerkstypen}
\label{table:chassis/transmission-types}
...
Graph mit Library / 2D Liste

\subsubsection{Bewegliche Hindernisse erfassen}
mittels Gewichtung (kein Hindernis=1, Hindernis=2) / ...

\subsubsection{Blockierte Knoten erfassen}
Markieren / Aus intern gespeichertem Netz entfernen

\subsubsection{Wegfindung} selber implementieren / Library verwenden (https://networkx.org/documentation/networkx-1.10/reference/generated/networkx.algorithms.shortest_paths.astar.astar_path.html , https://pypi.org/project/astar/)

\subsubsection{Zielauswahl}
Human input / Random / nach Berechnung Roboter waehlt selber

\subsubsection{Clientseitige Kommunikation (I/O)}
GUI, CLI, keine (Fehlermeldung wenn Ziel nicht erreicht)
