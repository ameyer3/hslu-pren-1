...
Bevor mit den Technologierecherchen begonnen wurde, wurden zum einen Kriterien definiert, die unser Roboter erfüllen soll, damit die Recherche eingeschraäkt wird und zum anderen wurde der Roboter in Teilfunktionen zerlegt.
...
\begin{tabularx}\textwidth{X X X X}
    ...
    Hebevorrichtung  & Antriebe & & Wegfindunge\\ 
    & Objekterkennung & & Zielauswahl\\ 
    & Steuerung &  & Clientseitiges I/O\\ 
\end{tabularx}
\end{table}
...
\textbf{Bilderkennung Rechner:} Nachfolgend werden einige Varianten verglichen, die als Rechner eingesetzt werden könnten. Auf diesem Rechner wird die Bilderkennungssoftware, sowie die Pfandfindung laufen.
...
\textbf{} & \textbf{Key-Value Pairs} & \textbf{2D Array}\\
  ...
  \hline
  \textbf{Vorteile}  & \makecell{-lightweight} & \makecell{-möglich in allen Sprachen} \\
  \hline
  \textbf{Nachteile} & \makecell{-je nach Sprache aufwändiger} & \makecell{} \\
  \hline
