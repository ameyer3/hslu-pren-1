...
Bevor mit den Technologierecherchen begonnen wurde, wurden zum einen Kriterien definiert, die unser Roboter efuellen soll, damit die Recherche eingeschraenkt werden konnte und zum anderen wurde der Roboter in Teilfunktionen zerlegt.

Als erstes wurden Muss-Kriterien definiert. Es wurde ein Brainstorming durchgefuehrt, um moegliche Formen des Roboters zu ermitteln. Dabei wurde versucht moeglichst offen zu sein und alle Moeglichkeiten in Betracht zu ziehen. Danach wurden einige Moeglichkeiten ausgeschlossen. Aus diesem Ausschlussverfahren konnte ermittelt werden, auf welche Kriterien wir fuer unseren Roboter Wert legen. Der Zweck dieser Kriterien ist es, die Technologierecherchen moeglichst fokussiert und schlank durchzufuehren.
Die Kriterien, die unser Roboter erfuellen soll, sind die folgenden:
\begin{itemize}
    \item Simpel
    \item Nicht zu teuer
    \item Nicht alle 30cmx30cmx80cm ausnutzen
    \item Robustheit
\end{itemize}

Als naechstes wurde der Roboter in Teilfunktionen zerlegt. Verschiedene Moeglichkeiten der einzelnen Teilfunktionen wurden recherchiert und in den folgenden Tabellen dargestellt. Dies sind die definierten Teilfunktionen:

TODO: Ueberabeiten nach anpassung der Technologierecherchen

\begin{itemize}
    \item Werkstoffe
    \item Fortbewegung
    \item Anordnung Räder
    \item Lenkung
    \item Hebevorrichtung
    \item Chassis
    \item Eingabeoptionen
    \item Ausgabeoptionen
...
Der Simulator wurde ebenfalls in Teilfunktionen zerlegt:

\begin{itemize}
    \item Wegenetz einlesen
    \item Wegenetz speichern
    \item Hindernisse erfassen
    \item Wegfindung
    \item Zielauswahl
    \item Clientseitige Kommunikation
\end{itemize}


\subsection{Mechanik}

Nachfolgend wurden aufgeteilt nach den wichtigsten Teilfunktionen; Werkstoffe, Fortbewegungsart, Anordnung der Räder, Lenkung, Hebevorrichtung und Chassis-Form bestehende Technoligien zusammengetragen und verglichen.
...
\usepackage{tabularx, makecell, url}
\usepackage{float}
\usepackage{hyperref}
...
% Tabelle 1: Fortbewegung
\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{} & \textbf{Differential Wheel Drive} & \textbf{Raupenfahrzeug} \\
  \hline
  \textbf{Beschreibung}  & 2 einzeln Angetriebene Räder, 1 Schwenkrad & FZ wird über Ketten/Raupen angetrieben \\
  \hline
  \textbf{Vorteile}  & \makecell{-kein Lenkgestänge\\-leicht\\-Punktwendung möglich} & \makecell{-Punktwenden möglich\\-geringer spez. Bodendruck\\-hohe Traktion\\-keine Lenkung nötig}  \\
  \hline
  \textbf{Nachteile} & \makecell{-instabil\\-2 Antriebsmotoren} & \makecell{-komplex\\-Gewicht\\-tiefe Vmax} \\
  \hline
  \textbf{Links} & \url{https://en.wikipedia.org/wiki/Differential_wheeled_robot} & \url{https://de.wikipedia.org/wiki/Kettenfahrzeug} \\
  \hline
\end{tabularx}
...
% Tabelle 2: Anordnung der Räder
\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{} & \textbf{Omniwheel} & \textbf{Überlagerungslenkgetriebe} \\
  \hline
  \textbf{Beschreibung}  & Rad, welches sich in der Ebene in alle Richtungen bewegen kann & "Panzerlenkung" 1 Motor mit 2 Ausgangswellen, Drehzahl/Richtung unabhängig einstellbar \\
  \hline
  \textbf{Vorteile}  & \makecell{-keine Lenkung nötig\\-scharfe Kurven möglich} & \makecell{-1 Antriebsmotor\\-1 Lenkmotor\\-kein Lenkgestänge}  \\
  \hline
  \textbf{Nachteile} & \makecell{-komplexer Aufbau} & \makecell{-Getriebe mit Verlusten\\-komplexer Aufbau} \\
  \hline
  \textbf{Links} & \url{https://en.wikipedia.org/wiki/Mecanum_wheel} & \url{https://de.wikipedia.org/wiki/Überlagerungslenkgetriebe} \\
  \hline
\end{tabularx}
\caption{Anordnung der Räder}
\label{table:wheel-arrangement}
\end{table}
...
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{} & \textbf{Lenkgetriebe} & \textbf{Überlagerungslenkgetriebe} \\
  \hline
  \textbf{Beschreibung}  & zur Umwandlung Rotation in unterschiedliche Einschlagwinkel & "Panzerlenkung" 1 Motor mit 2 Ausgangswellen, Drehzahl/Richtung unabhängig einstellbar \\
  \hline
  \textbf{Vorteile}  & \makecell{-simpler Aufbau\\-1 Aktuator} & \makecell{-1 Antriebsmotor\\-1 Lenkmotor\\-kein Lenkgestänge}  \\
  \hline
  \textbf{Nachteile} & \makecell{-begrenzter Einschlag} & \makecell{-Getriebe mit Verlusten\\-komplexer Aufbau} \\
  \hline
  \textbf{Links} & \url{https://de.wikipedia.org/wiki/Lenkgetriebe} & \url{https://de.wikipedia.org/wiki/Überlagerungslenkgetriebe} \\
  \hline
\end{tabularx}
\caption{Lenkung}
...
% Tabelle 4: Hebevorrichtung
\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{} & \textbf{Hebevorrichtung A} & \textbf{Hebevorrichtung B} \\
  \hline
  \textbf{Beschreibung}  & Beschreibung von Hebevorrichtung A & Beschreibung von Hebevorrichtung B \\
  \hline
  \textbf{Vorteile}  & \makecell{-Vorteil A1\\-Vorteil A2} & \makecell{-Vorteil B1\\-Vorteil B2}  \\
  \hline
  \textbf{Nachteile} & \makecell{-Nachteil A1\\-Nachteil A2} & \makecell{-Nachteil B1\\-Nachteil B2} \\
  \hline
  \textbf{Links} & \url{https://example.com/hebeb-a} & \url{https://example.com/hebeb-b} \\
  \hline
\end{tabularx}
...
Es gibt verschiedneste Arten einen Roboter autonom fortzubewegen. Aufgrund der vielen Möglichkeiten wurden bei einem Brainstorming im Team Varianten die zu Komplex oder aufwendig in der Umsetzung sind bereits ausgeschlossen, ohne Recherche zu betreiben. Unten aufgeführt sind die Varianten, welche weiter verfolgt wurden.
...
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
  \textbf{} & \textbf{Ultraschall} & \textbf{Optoelektrisch} & \textbf{Lidar}\\
  \hline
...
\textbf{} & \textbf{Lidar} & \textbf{} \\
  ...
  \hline
  \textbf{Links} & \url{https://de.wikipedia.org/wiki/Lidar} & \\
  \hline
