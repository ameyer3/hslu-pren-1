...
\addcontentsline{toc}{subsection}{Technologierecherchen}
...
Bevor mit den Technologierecherchen begonnen wurde, wurden die Anforderungen definiert. So konnte die Recherche eingeschränkt und die Teilfunktionen bestimmt werden.

Als erstes wurden Muss-Kriterien definiert. Es wurde ein Brainstorming durchgeführt, um mögliche Formen des Roboters zu ermitteln. Dabei wurde versucht möglichst offen zu sein und alle Möglichkeiten in Betracht zu ziehen. Danach wurden einige Möglichkeiten ausgeschlossen. Aus diesem Ausschlussverfahren konnte ermittelt werden, auf welche Kriterien das Team für den Roboter Wert legt. Der Zweck dieser Kriterien ist es, die Technologierecherchen möglichst fokussiert und schlank durchzuführen.
Die Kriterien, die der Roboter erfüllen soll, sind die folgenden:
...
\addcontentsline{toc}{subsubsection}{Mechanik}
...
Es gibt verschiedenste Arten einen Roboter autonom fortzubewegen. Aufgrund der vielen Möglichkeiten wurden bei einem Brainstorming Varianten, die zu komplex oder aufwendig in der Umsetzung sind, bereits ausgeschlossen, ohne Recherche zu betreiben. Eine solche komplexe Variante sind beispielsweise Beine. Unten aufgeführt sind die beiden Varianten, welche weiter verfolgt wurden.
...
\addcontentsline{toc}{subsubsection}{Steuerung}


Im nachfolgenden Abschnitt werden die, für die Steuerung notwendigen elektrischen Komponente und Systeme. aufgezeigt und verglichen.  Ein- und Ausgabemethoden, Stromversorgung, Antriebe, Objekterkennung und die Steuerung sind die wichtigsten Bestandteile der Steuerung, um den Anforderungen gerecht zu werden. Damit die einzelnen Schnittstellen zur Mechanik und Navigation gewährleistet sind, benötigt es die passenden Komponente.
...
Für die Antriebsfunktion benötigt es mehrere Antriebe mit unterschiedlichen Eigenschaften. Zum einen braucht es welche für die Fortbewegung des Fahrzeugs und zum anderen für das Heben des Hindernisses. Diese sind in der Tabelle aufgezeigt.
...
Um die Objekte zu erkennen, welche auf dem Wegnetzwerk stehen, werden zwei verschiedene Technologien verglichen.
...
Da der Roboter beim Traversieren des Graphens einer Linie folgen muss, muss er in der Lage sein diese Linien zu erkennen. 
...
Die Logik der Steuerung muss auf einem Computer laufen. Zwei Varianten wurden hier verglichen.
...
Damit ein Not-Stop durchgeführt werden kann, kann ein Not-Aus Pilztaster oder ein normaler Schalter verwendet werden.
...
\textbf{Bilderkennung Software:} Nachfolgend werden einige Softwarelösungen verglichen, mit denen eine Bilderkennung durchgeführt werden könnten. Dies wird benötigt, um Pylonen, Hindernisse und fehlende Streckenteile zu erkennen.
...
\textbf{Bilderkennung Kamera:} Nachfolgend werden einige Kamera verglichen, die verwendet werden könnten, um die Bilder für die Bilderkennung zu schiessen.
...
\textbf{Bilderkennung Rechner:} Nachfolgend werden einige Rechner verglichen, die eingesetzt werden könnten. Auf diesem Rechner wird die Bilderkennungssoftware, sowie die Berechnung für die Pfadfindung laufen.
...
\addcontentsline{toc}{subsubsection}{Simulator}


Möglichkeiten für die Teilfunktionen des Simulators sind im folgenden Teil aufgelistet und verglichen. Die Teilfunktionen des Simulators sind das Wegenetz einlesen und speichern, Hindernisse erfassen, Wegfindung und ein Client User Interface.
...
Damit die beweglichen Hindernisse im intern gespeicherten Netzwerk erfasst werden können, wurden folgende Methoden bewertet.
...
Damit die blockierenden Hindernisse im gespeicherten Netzwerk erfasst werden können, wurden folgende Methoden bewertet.
...
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{} & \textbf{Graphical User Interface (GUI)} & \textbf{Command Line Interface (CLI}\\
  \hline
...
\begin{tabularx}{\textwidth}{|l|X|X|}
\hline
\textbf{} & \textbf{Keine Inputs oder Outputs} & \textbf{Text-Based User Interface (TUI)}\\
  \hline
  \textbf{Beschreibung} & Der Mensch kommuniziert nicht mit dem Roboter. Falls der Roboter das Ziel nicht erreicht wird eine Exception raised. &  Outputs werden über ein Text-Based User Interface dargestellt.\\
  \hline
